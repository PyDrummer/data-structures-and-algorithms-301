...
const toTitleCase = (arr) => {
  // Solution code here...
  let titled = arr.map(word => {
    let first = word.charAt(0).toUpperCase();
    let rest = word.substring(1, 10);
...
let biggerThanLuke = (arr) => {
  ...
  arr.forEach(person => {
    ...
    //console.log('luke\'s mass', lukeMass);
    if (parseInt(person.mass) > lukeMass) {
      //console.log(person.name);
      mass.push(person.name);
    }
  });

  let massString = mass.toString();
  let cleanString = massString.replace(',', ' - ');
