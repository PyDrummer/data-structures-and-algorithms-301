...
const findHappiness = (arr) => {
  ...
  let smilesOnly = [];
  arr.forEach(smiles => {
    if (smiles.includes(':)') && smiles.includes(':)') !== undefined) {
      //console.log('smiles', smiles);
      smilesOnly.push(smiles);
    }
  });
  return smilesOnly;
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  let standNumb = arr.map(num => {
    let firstThree = num.substring(1, 4);
    let theMid = num.substring(6, 9);
    let theLast = num.substring(10, 14);
    return firstThree+theMid+theLast;
  });
  return standNumb;
};
