...
function getCurrentEvents(request, response) {
  // Solution code here...
  let currentEventRes = mapCurrentEvents();
  response.send(200).json(currentEventRes);
}

const mapCurrentEvents = () => {
  // Solution code here...
  let newEvent = currentEvents.news.map(event => {
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let arrLen = arr.reduce((count, value, indx) =>{
    if (value) {count ++;}
    return count;
  }, 0);
  return arrLen;
