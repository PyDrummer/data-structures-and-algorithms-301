...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach((person) => {
    ...
    let $template = $(`<section>${html}</section>`);
...
const listFoods = (recipe) => {
  let result = [];
  // Solution code here...
  //check for the words that need to be taken out, slice those words out.
  recipe.ingredients.forEach((food) => {
    if (food.includes('sized')) {
      let size = food.indexOf('sized');
      result.push(food.slice(size + 6, food.length));
    }
    else if (food.includes('pounds')){
      let pounds = food.indexOf('pounds');
      result.push(food.slice(pounds + 7, food.length));
    }
    else if (food.includes('gallons')){
      let gal = food.indexOf('gallons');
      result.push(food.slice(gal + 8, food.length));
    }
    else if (food.includes('pound')){
      let lb = food.indexOf('pound');
      result.push(food.slice(lb + 6, food.length));
    }
    else if (food.includes('cups')){
      let cups = food.indexOf('cups');
      result.push(food.slice(cups + 5, food.length));
    }
  });
  return result;
};
