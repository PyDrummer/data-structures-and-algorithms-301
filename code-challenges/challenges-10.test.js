...
const createServer = () => {
  ...
  app.get('/aboutme', (request, response) => {
    response.send('Hello, my name is Anthony!');
  });

  app.get('/favoritefoods', (request, response) => {
    response.send(['apples', 'pizza', 'ice-cream']);
  });
...
const count = (target, input) => {
  ...
  // using .map
  let counter = 0;
  let count = input.map(num => {
    let newInput = num.map(val => {
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  input.forEach(index => {
    let test = index.reduce((newVal, currentVal) => {
      if ( currentVal % 5 === 0 && typeof(currentVal) === 'number') {
        newVal.push(Math.pow(2, currentVal));
      }
      return newVal;
    }, []);
    newTwoD.push(test);
